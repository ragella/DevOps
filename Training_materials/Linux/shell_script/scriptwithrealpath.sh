...
#Function to get absolute path of the directory
rootdir () {
	absrootdir=$(realpath "$1")	#realpath command is used to find the absolute path of a file/directory
#Checking if realpath completed successfully
	if [ $? != 0 ]; then 		# $?Checks the exit status of previous command
		exit 1
	fi
}
